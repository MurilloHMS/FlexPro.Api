...
namespace FlexPro.Infrastructure.Templates.Reports
{
    public class FuelSupplyReport : IDocument
    {
        ...
        public FuelSupplyReport(List<Abastecimento> abastecimentos, byte[] logoImage, byte[] postoImage,AbastecimentoMetricsResult metricsResult)
        {
            ...
            container.Padding(20).Column(col =>
            {
                foreach (var departamento in _departmentMetrics)
                {
                    ...
                    col.Item().Background(Colors.Grey.Lighten3).Padding(10).Column(c =>
                    {
                        c.Spacing(5);
                        c.Item().Text("Métricas").FontSize(14);
                        c.Item().Text(_metricsResult.MetricasPorDepartamento.TryGetValue(departamento.Departamento, out var metricas) ? metricas : "Sem métricas disponíveis").FontSize(ReportStyles.DefaultFontSize);
                    });
                }
            });
        }
