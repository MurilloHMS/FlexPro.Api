...
public class FuelSuppy_Report : IDocument
{
    ...
    public FuelSuppy_Report(AbastecimentoService service, IAbastecimentoRepository repository,
        List<Abastecimento> abastecimentos, byte[] logoImage, byte[] postoImage)
    {
            ...
            col.Item().Row(row =>
            {
                row.ConstantColumn(ReportConstants.ChartColumnWidth).Column(col =>
                {
                    ...
                    col.Item().AspectRatio(2).Svg(size =>
                        ChartGenerator.GenerateBarChartSvg(_departmentMetrics, d => d.Litros, d => d.Departamento,
            ...
            col.Item().Row(row =>
            {
                ...
                row.ConstantColumn(270).Column(col =>
                {
                    ...
                    col.Item().AspectRatio(2).Svg(size =>
                    {
                        ...
                        var departamentosCount = dados.Count;
                        var barSpacing = 1.0;
                        var barWidth = 0.3;

                        var xBase = Enumerable.Range(0, departamentosCount).Select(i => (double)i).ToArray();
    ...
    private void ComposeContentSetor(IContainer container)
    {
        ...
        container.Padding(20).Column(col =>
        {
            ...
            {
                col.Item().PaddingBottom(10).Text($"Abastecimento Setor - {departamento.Departamento}").FontSize(24)
                    .Bold().AlignCenter();

                var funcionarios = _abastecimentosLista.Where(x => x.Departamento == departamento.Departamento)
                    .GroupBy(y => y.NomeDoMotorista).Select(x => new
                    {
                        Funcionario = x.Key,
                ...
                col.Item().Row(row =>
                {
                    row.ConstantColumn(500).Column(col =>
                    {
                        ...
                        col.Item().AspectRatio(4).Svg(size =>
                        {
                            ...
                            var funcionariosCount = funcionarios.Count;
                            var barSpacing = 1.0;
                            var barWidth = 0.3;

                            var xBase = Enumerable.Range(0, funcionariosCount).Select(i => (double)i).ToArray();
                            ...
                            foreach (var bar in barsLitro.Bars) bar.Label = bar.Value.ToString("N0");
                ...
                col.Item().Row(row =>
                {
                    row.ConstantColumn(500).Column(col =>
                    {
                        ...
                        col.Item().AspectRatio(4).Svg(size =>
                        {
                            ...
                            foreach (var bar in barsLitro.Bars) bar.Label = bar.Value.ToString("F2");
                ...
                col.Item().Row(row =>
                {
                    row.ConstantColumn(500).Column(col =>
                    {
                        ...
                        col.Item().AspectRatio(4).Svg(size =>
                        {
                            ...
                            var barsLitro = plot.Add.Bars(xBase,
                                funcionarios.Select(d => d.DistanciaPercorrida).ToArray());
