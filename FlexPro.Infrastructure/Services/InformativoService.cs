                    ...
                    var fileData = worksheet.RowsUsed()
                        ...
                        .Select(row => new InformativoNFe
                        {
                            ...
                            Data = row.Cell(2).TryGetValue<string>(out var dataString) &&
                                   DateTime.TryParseExact(dataString, "dd/MM/yyyy", CultureInfo.InvariantCulture,
                                       DateTimeStyles.None, out var dataFormatada)
                                ? dataFormatada
                                : default,
                            CodigoCliente = row.Cell(3).TryGetValue<string>(out var codigoCliente)
                                ? codigoCliente
                                : default!,
                            NomeDoCliente = row.Cell(4).TryGetValue<string>(out var nomeCliente)
                                ? nomeCliente
                                : default!,
                            CodigoProduto = row.Cell(6).TryGetValue<string>(out var codigoProduto)
                                ? codigoProduto
                                : default!,
                            TipoDeUnidade = row.Cell(7).TryGetValue<char>(out var tipoDeUnidade)
                                ? tipoDeUnidade
                                : default,
                            NomeDoProduto = row.Cell(8).TryGetValue<string>(out var nomeProduto)
                                ? nomeProduto
                                : default!,
                            Quantidade = row.Cell(9).TryGetValue<double>(out var quantidade) ? quantidade : default,
                            ValorTotalComImpostos = row.Cell(10).TryGetValue<decimal>(out var valorTotal)
                                ? valorTotal
                                : default
                        }).ToList();
                    ...
                    var fileData = worksheet.RowsUsed()
                        ...
                        .Select(row => new InformativoOS
                        {
                            ...
                            CodigoCliente = row.Cell(2).TryGetValue<string>(out var codigoCliente)
                                ? codigoCliente
                                : default!,
                            DataDeAbertura = row.Cell(4).TryGetValue<DateTime>(out var dataAbertura)
                                ? dataAbertura
                                : default,
                            DataDeFechamento = row.Cell(5).TryGetValue<DateTime>(out var dataFechamento)
                                ? dataFechamento
                                : default,
            ...
            using (var stream = new MemoryStream())
            {
                ...
                using (var workbook = new XLWorkbook(stream))
                {
                    ...
                    var fileData = worksheet.RowsUsed()
                        ...
                        {
                            CodigoCliente = row.Cell(1).TryGetValue<string>(out var codigoCliente)
                                ? codigoCliente
                                : default!,
        ...
        try
        {
            ...
            foreach (var cliente in parceiros)
            {
                ...
                var informativo = new Informativo
                {
                    ...
                    MediaDiasAtendimento = clienteOsInfo != null && clienteOsInfo.Any()
                        ? clienteOsInfo.Sum(x => x.DiasDaSemana)
                        : 0,
